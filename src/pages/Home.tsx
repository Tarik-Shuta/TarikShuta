const projects = [
  ...
  },
  {
    ...
    title: 'EClubs',
    description:
      'Use this space to briefly explain the product, your contribution, and the result.',
  ...
  },
  {
    ...
    title: 'Project name',
    description:
      'Add another project that shows a different skill or the kind of work you want to do more of.',
...
function Home() {
  return (
    <main className="overflow-hidden">
      <div className="mx-auto w-[calc(100%-2rem)] max-w-[1180px] sm:w-[calc(100%-3rem)]">
        <nav className="flex items-center justify-between py-7" aria-label="Main navigation">
          <a className="text-lg font-bold tracking-[-0.05em] no-underline" href="#top">
            Tarik<span className="text-accent">.</span>
          </a>
          <a
            className="font-mono text-xs font-bold tracking-[0.1em] text-muted uppercase transition hover:text-accent"
            href="#contact"
          >
            Let&apos;s talk ↗
          </a>
        ...
        <section id="top" className="flex min-h-[calc(100vh-84px)] flex-col justify-center py-20">
          ...
          <p className="mt-7 max-w-2xl text-lg leading-relaxed text-muted sm:text-xl">
            I&apos;m a software engineering student at the International University of Sarajevo, interested in
            creating useful digital experiences and becoming a better developer with every project.
          </p>
        ...
        <section id="projects" className="border-t border-white/10 py-20 sm:py-28">
          <div className="mb-11 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <p className="font-mono text-xs font-bold tracking-[0.14em] text-accent uppercase">Selected work</p>
              <h2 className="mt-3 text-4xl font-medium tracking-[-0.06em] sm:text-5xl">Projects I&apos;ve built.</h2>
            </div>
            <p className="max-w-sm text-sm leading-relaxed text-muted">
              I&apos;ll keep this section updated as I build and improve things.
            </p>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {projects.map((project) => (
              <article
                key={project.number}
                className="group relative overflow-hidden rounded-2xl border border-white/10 bg-surface p-7 transition duration-200 hover:-translate-y-1 hover:border-accent/40 sm:p-8"
              >
                <div className="absolute -right-16 -bottom-20 size-52 rounded-full bg-evergreen/30 blur-sm transition group-hover:bg-accent/15" />
                ...
                <ul className="relative mt-7 flex flex-wrap gap-2" aria-label="Technologies used">
                  {project.tags.map((tag) => (
                    <li key={tag} className="rounded-md border border-white/10 bg-white/[0.025] px-2 py-1 font-mono text-[0.68rem] text-muted">
                      {tag}
                    </li>
                  ))}
                </ul>
        ...
        <section id="about" className="grid gap-10 border-t border-white/10 py-20 sm:grid-cols-[0.8fr_1.2fr] sm:py-28">
          <p className="font-mono text-xs font-bold tracking-[0.14em] text-accent uppercase">About me</p>
          <div>
            ...
            <p className="mt-6 max-w-xl leading-relaxed text-muted">
              I am a Software Engineering student passionate about web development and building practical software
              solutions. Experienced in developing full-stack applications with modern technologies, including frontend,
              backend, databases, and APIs. I enjoy solving problems, learning new technologies, and continuously
              improving my skills while contributing to meaningful projects and growing as a developer.
            </p>
          </div>
        </section>

        <section id="contact" className="border-t border-white/10 py-20 sm:py-28">
          <p className="font-mono text-xs font-bold tracking-[0.14em] text-accent uppercase">Get in touch</p>
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <a
              className="inline-flex rounded-full bg-accent px-5 py-3 text-sm font-bold text-canvas no-underline transition hover:-translate-y-0.5 hover:bg-accent-bright"
              href="mailto:tariksuta4@gmail.com"
            >
            tariksuta4@gmail.com ↗
            </a>
            <a
              className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-bold text-canvas no-underline transition hover:-translate-y-0.5 hover:bg-accent-bright"
              href="https://github.com/Tarik-Shuta"
              target="_blank"
              rel="noreferrer"
              aria-label="Visit Tarik Suta's GitHub profile"
            >
